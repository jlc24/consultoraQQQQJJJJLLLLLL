...
class ImprimirReporte
{
    ...
    {
        $stmt = Conexion::Conectar()->prepare("SELECT *from detalle_factura WHERE fac_id = (SELECT MAX(fac_id) FROM factura)-1");
        $stmt->execute();
...
$sql="SELECT * FROM factura WHERE fac_id = (SELECT MAX(fac_id) FROM factura)-1";
