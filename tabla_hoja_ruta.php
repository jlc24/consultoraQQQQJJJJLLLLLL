        ...
        <table id="hoja_ruta_historial" class="table mb-0 table-sm table-bordered dt-responsive" width="100%">
            ...
            <tbody>
                <?php
                    $all = "SELECT proceso_id FROM configuracion";
                    $res = mysqli_query($conexion, $all);
                    $r = mysqli_fetch_assoc($res);
                    if ($r['proceso_id'] != 'all') {
                        $sql="SELECT hoja_id, hoja_numero_tramite, hoja_demandante, hoja_demandado, hoja_patrocinio, hoja_tipo_proceso, proceso_nombre FROM hoja, proceso WHERE hoja.hoja_area_proceso = proceso.proceso_nombre AND proceso.proceso_id = (SELECT proceso_id FROM configuracion) ORDER BY hoja_numero_tramite DESC;";
                        ...
                        $conproc = "SELECT proceso_id FROM asignacion WHERE asig_estado != '0' AND adm_id = '".$admid."';";
                        $r = mysqli_query($conexion,$conproc);
                        while ($a = mysqli_fetch_assoc($r)) {
                            $sql="SELECT hoja_id, hoja_numero_tramite, hoja_demandante, hoja_demandado, hoja_patrocinio, hoja_tipo_proceso, proceso_nombre FROM hoja, proceso WHERE hoja.hoja_area_proceso = proceso.proceso_nombre AND proceso.proceso_id = '".$a['proceso_id']."' ORDER BY proceso_nombre DESC;";
                            $resultado=mysqli_query($conexion,$sql);
                            while($registro = mysqli_fetch_assoc($resultado)){
                                ?>
                                    <tr>
